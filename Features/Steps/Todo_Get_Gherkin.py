...
def Todo_step_given_todo_item(context, content):
...
def Todo_step_given_todo_items(context):
...
def Todo_step_when_create_todo_item(context, content):
    # Create a todo
    response = requests.post(BASE_URL, json=eval(content))
    context.response = response

@when(u'I search for that todo item')
def Todo_step_when_search_todo_item(context):
    # Get a todo item
    response = requests.get(TODO_ID_URL.format(id = context.todoID))
    context.response = response

@when(u'I search for a todo item with a bad id')
def Todo_step_when_search_todo_item_bad_id(context):
...
def Todo_step_when_delete_non_existing_todo_item(context):
    # Creating item using ID 123456789 as it will never exist in testing
    response = requests.delete(TODO_ID_URL.format(id = 123456789))
    context.response = response

@when(u'I add a category to a non-exsiting todo item with {content}')
def Todo_step_when_add_category_to_non_existing_todo_item(context,content):
    # Creating item using ID 123456789 as it will never exist in testing
...
def Todo_step_when_add_category(context, content):
...
def Todo_step_then_todo_item_created(context):
    # Make sure the todo item is created
    assert context.response.status_code == 201

@then(u'the todo item should contain {content}')
def Todo_step_then_todo_item_contains(context, content):
...
def Todo_step_then_category_created(context):
    # Make sure the category was created
    assert context.response.status_code == 201

@then(u'the category should contain {content}')
def Todo_step_then_category_contains_title(context, content):
